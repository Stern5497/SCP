...
def get_data(training_args, data_args, model_args, download_mode, experimental_samples=False):
    ...
    if training_args.do_train:
        if experimental_samples == True:
            train_dataset = load_dataset("Stern5497/SCP", data_args.finetuning_task, split='train[:5%]',
        ...
        if bool(re.search('eurlex', data_args.finetuning_task)):
            raise Exception("finetuning_task eurlex not allowed in SBCP")
        if data_args.finetuning_task in ner_tasks:
            raise Exception(f"{data_args.finetuning_task} not allowed in SBCP")

    if training_args.do_eval:
        if experimental_samples == True:
            eval_dataset = load_dataset("Stern5497/SCP", data_args.finetuning_task, split='validation[:5%]',
                                        cache_dir="datasets_caching", download_mode=download_mode)
        elif experimental_samples == False:
            eval_dataset = load_dataset("Stern5497/SCP", data_args.finetuning_task, split='validation[:5%]',
                                        cache_dir="datasets_caching", download_mode=download_mode)
    ...
    if training_args.do_predict:
        if experimental_samples == True:
            predict_dataset = load_dataset("Stern5497/SCP", data_args.finetuning_task, split='test[:5%]',
                                           cache_dir="datasets_caching", download_mode=download_mode)
        elif experimental_samples == False:
            predict_dataset = load_dataset("Stern5497/SCP", data_args.finetuning_task, split='test[:5%]',
                                           cache_dir="datasets_caching", download_mode=download_mode)
